...
namespace Charlotte
{
	public class Canvas
	{
		...
		/// <returns>新しいキャンパス</returns>
		public Canvas DrawString(string text, int fontSize, string fontName, I4Color color, int x, int y)
		{
			Bitmap bmp = this.ToBitmap();

			using (Graphics g = Graphics.FromImage(bmp))
			{
				g.DrawString(text, new Font(fontName, fontSize, FontStyle.Regular), new SolidBrush(color.ToColor()), new Point(x, y));
			}
			return Load(bmp);
		}
